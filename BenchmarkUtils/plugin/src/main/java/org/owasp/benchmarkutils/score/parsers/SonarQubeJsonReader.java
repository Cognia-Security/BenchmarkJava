...
public class SonarQubeJsonReader extends Reader {
    ...
    private TestCaseResult parseSonarQubeQualityIssue(JSONObject finding) {
        try {
            String component = finding.getString("component");
            String filename = component.replaceAll("\\\\", "/");
            ...
            if (filename.contains(BenchmarkScore.TESTCASENAME)) {
                ...
                String key = finding.optString("key", "");
                String message = finding.optString("message", "");
                String baseName = Reader.extractFilenameWithoutEnding(component);
                if (!key.isEmpty()) {
                    tcr.setTestCaseName(baseName + " (id: " + key + ")");
                } else {
                    tcr.setTestCaseName(baseName);
                }
                if (!key.isEmpty()) {
                    tcr.setEvidence(
                            "id: " + key + (message.isEmpty() ? "" : " | message: " + message));
                } else {
    ...
    private TestCaseResult parseSonarQubeHotSpotIssue(JSONObject finding) {
        try {
            ...
            if (filename.contains(BenchmarkScore.TESTCASENAME)) {
                ...
                String probability = finding.optString("vulnerabilityProbability", "");
                StringBuilder evidence = new StringBuilder();
                if (!key.isEmpty()) {
                    evidence.append("id: ").append(key);
                }
                if (!probability.isEmpty()) {
                    if (evidence.length() > 0) {
                        evidence.append(" | ");
                    }
                    evidence.append("vulnerabilityProbability: ").append(probability);
                }
                tcr.setEvidence(evidence.length() > 0 ? evidence.toString() : "");
                return tcr;
            }
